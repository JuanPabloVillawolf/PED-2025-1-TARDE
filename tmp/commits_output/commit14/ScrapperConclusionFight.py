...
for url in urls:
    ...
    fighter_1_significant_strikes_per = fighters_significant_strikes_per[0].get_text(strip=True) if len(fighters_significant_strikes_per) > 0 else pd.NA
    fighter_2_significant_strikes_per = fighters_significant_strikes_per[1].get_text(strip=True) if len(fighters_significant_strikes_per) > 1 else pd.NA
    fighters_total_strikes = row_cells[4].find_all("p") if len(row_cells) > 4 else []
    fighter_1_total_strikes = fighters_total_strikes[0].get_text(strip=True) if len(fighters_total_strikes) > 0 else pd.NA
    fighter_2_total_strikes = fighters_total_strikes[1].get_text(strip=True) if len(fighters_total_strikes) > 1 else pd.NA
    fighters_takedowns_per = row_cells[6].find_all("p") if len(row_cells) > 5 else []
    fighter_1_takedowns_per = fighters_takedowns_per[0].get_text(strip=True) if len(fighters_takedowns_per) > 0 else pd.NA
    fighter_2_takedowns_per = fighters_takedowns_per[1].get_text(strip=True) if len(fighters_takedowns_per) > 1 else pd.NA
    fighters_submission_attempts = row_cells[7].find_all("p") if len(row_cells) > 6 else []
    fighter_1_submission_attempts = fighters_submission_attempts[0].get_text(strip=True) if len(fighters_submission_attempts) > 0 else pd.NA
    fighter_2_submission_attempts = fighters_submission_attempts[1].get_text(strip=True) if len(fighters_submission_attempts) > 1 else pd.NA
    ...
    print(f"\tAdding stats from fight: {fighter_1_name} vs {fighter_2_name}")
